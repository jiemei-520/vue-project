<template>
  <div class="detail">
    ...
    <van-goods-action>
      <van-goods-action-icon :to="{name: 'Shopbag'}" icon="bag" text="购物袋" :badge="bagCount == 0 ? '' : bagCount" :color="bagCount > 0 ? '#d9baba' : '#646566'" />
      <van-goods-action-icon icon="like" :text="isLike ? '已收藏' : '未收藏'" :color="isLike ? '#d9baba' : '#646566'" @click="toggleLikeProduct" />
      <van-goods-action-button text="加入购物袋" color="#d9DDb0" @click="addShopbag()" />
      <van-goods-action-button text="立即购买" color="#d9baba" @click="buy(true)" />
    </van-goods-action>
